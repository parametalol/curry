...
func DropLastOf{{.N}}[{{template "Gen" .}}]({{range $i, $a := .Args}}{{if ne $i 0}}, {{end}}{{$a.Name}} T{{$i}}{{end}}){{if ne $.N 1}} {{end}}{{if gt $.N 2}}({{end}}{{- range $i, $a := .Args}}{{if ne $i $.Last}}{{if ne $i 0}}, {{end}}T{{$i}}{{end}}{{end}}{{if gt $.N 2}}){{end}} {
